...
This chapter details the work that was published in \cite{johnson_radio_2025} which conducted out a search for \textit{Fermi} redback candidate 1FGL J0523.5$-$2529. The paper itself remains almost verbatim to the final published work but has been slightly edited for readability while some minor repetition remains (i.e. introduction of radiometer equation) it was left in intentionally so it is clear how pseudo-luminosities were calculated specific to this project. As will be noted by the reader later in the thesis while radiometer equation remains largely the same as described in \cref{chap:radioastro} there are some slight adjustments based on use case. The next section gives a brief introduction for pulsar searching software just to give context is being referred to when `periodically' or `single burst' searching which is prevalent throughout this thesis. 

\section{Example of Pulsar Search Pipeline}

A zoo of pulsar software has been developed over the past decades to detect and time pulsars. One of the most commonly used pieces of software in the search for pulsars is PRESTO \citep{ransom_new_2001}. While there are many software packages that search for pulsars and have their own merits for the purposes of this introduction \textsc{PRESTO} is used. An example of a search pipeline with \textsc{presto} is illustrated in \cref{fig: presto-work-flow}, but as with the software this can be rearranged and various stages can be swapped out for alternative routines (i.e. rfifind). Due to the large data volumes produced by modern radio telescopes, the majority of pulsar searches are carried out in a distributed manner on high performance computing clusters such as OzGrav.

\begin{figure}[h]
    \centering
    \includegraphics[width = 0.9\textwidth]{chapter3/figures/presto_pipeline.pdf}
    \caption{Outline of the PRESTO search strategy used to search for binary pulsars.}
    \label{fig: presto-work-flow}
\end{figure}


The first step in nearly all radio observations is to remove Radio Frequency Interference (RFI) from the observational data, this is usually caused by most modern technologies. This is important as artificial signals can saturate a specific channel(s) dominating the power spectrum and ruining dynamic range. Some transmitters can mimic periodic signals associated with pulsar emission which also must be removed. This is usually done by creating a frequency and temporal mask.  

\texttt{rfifind} which is part of the PRESTO suite searches in both frequency and time domains. It analyses each channel for a specified time integration (or block). Firstly the time domain statistics are computed which consists of the mean and standard deviation of the values in each channel. For blocks where the mean value exceeds $4\sigma$ the block is flagged as RFI. If more than 30\% of the channel is flagged the entire channel is masked completely and replaced with a median constant band-pass value. But the thresholds can be modified as needed. 

Following the masking of all observation files the next step is then dedisperse the data. This is done to remove the effects of dispersion caused by the interstellar medium as discussed in \cref{sec:DispersionMeasure}.  When correcting for dispersion it is important to consider the possible range of DMs that the pulsar could have. The DM can be estimated through the use of density electron models such as \cite{ocker_ne2025_2026} and \cite{yao_new_2017} as mentioned in \cref{chap:introduction}. In cases where pulsars or other transients are being searched for blindly a `DM plan' is made based on the telescope configuration to decide on what DM range and DM trials are reasonable for a given survey. For full discussion on this please see \cref{appen:low-frequency-dd} and \cref{chap:LOFTS}. However, when dedisperseing any observation it inherently induces smearing in the data, which can be a problem when searching for things like MSPs and FRBs. There are four types of smearing that need to be accounted for and characterized by the following equation, 

\begin{equation}
    \tau_{\text{total}} = \sqrt{\tau_{\text{samp}}^2 + \tau_{\text{sub}}^2 + \tau_{\text{BW}}^2 + \tau_{\text{chan}}^2}
\end{equation}

Where $\tau_\text{chan}$ is the channel smearing, $\tau_\text{samp}$ is the sample time, $\tau_\text{sub}$is the subband smearing and $\tau_\text{BW}$ is the bandwidth smearing. For sources with a known DM coherent dedispersion can be employed to mitigate dispersion effects by employing the complex conjugate of the interstellar medium's transfer function to deconvolve the data before undergoing filterbanking. In less technical jargon this means that because the effects of the ISM are know to effect signals at a given distance am additional function when processing raw voltages can be applied to correct for it. By doing this the highest possible bandwidth is retained and all intra-channel smearing is removed \citep{hankins_pulsar_1975}. The reason this can't be used for blind searches is the increased computational resources needed to process raw voltages, whereas sacrificing the trade-off in smearing is worth it in most cases. 


If searching for a periodic transient such as a pulsar the data then needs to be transformed into the frequency domain to search for the most dominant periodic signals. This is commonly done with a Discrete Fourier Transform (DFT) in pulsar astronomy and computationally carried out using a Fast Fourier Transform (FFT). For a time series, $S_j$ of a given length of $N$ it is necessary to convert the time series into the barycentric frame of reference. The barycentric frame if reference point is centred on the mass of the solar system. \\ The $k$th Fourier component of the time series is described in \citet[pp.~132--134]{pulsar_handbook} as,

\begin{equation}
    \mathcal{F}_k = \sum_{j=0}^{N-1} S_j \exp\left(-2\pi \sqrt{-1} \frac{jk}{N}\right)
\end{equation}

To complete this computation it requires $N^2$ floating point operations. However, if a FFT operation is employed the number of operations is reduced to $N \log_2 N$ for a time series of length $N$. Since the time series data are real numbers symmetry can be exploited as the DFT is symmetric about the Nyquist frequency, $\nu_{\text{Nyq}} = 1/(2 t_{\text{samp}})$. For any frequencies higher than the $\nu_{\text{Nyq}}$ the complex conjugates the same as the corresponding lower half of the frequency.

Once the strong candidates have been identified the next step is to fold the observations to each candidate expected period. Pulsar folding is a technique commonly used in searching if single pulses are not bright enough to be clearly visible in the observed time series. The term folding comes from the fact that the time series is folded on itself by some trial period informed by the search in the Fourier domain. In the case a pulsar is detected it will look something like \cref{fig:folding}. 

\begin{figure}
    \centering
    \includegraphics[width = 0.9\textwidth]{chapter3/figures/folding.png}
    \caption[Illustration of puslar folding.]{Adapted illustration from \cite{folding_picture} of the folding process. The left panel shows the observed time series, the middle panel shows the folded time series and the right panel shows the summed pulse profile.}
    \label{fig:folding}
\end{figure}

In \textsc{PRESTO}'s case a routine called \texttt{prepfold} is used for each promising candidates over a certain noise floor. These candidates are then folded with each folded profile examined by eye for possible positive pulsar detection (see cref{fig: prepfold}). This plot contains integrated pulse profile, time series plot and sub-band plot. It also shows DM, $P$ and $\dot P$ plots as a function of reduced $\chi^2$ values. Each of these plots must be sifted through by eye. If a signal is a genuine pulsar it will exhibit distinct peaks in DM, $P$ and $\dot P$ plot. It will also show a `DM island', a real pulsar produces a distinct DM island or peak centred on its true DM. At incorrect DMs, the pulse becomes smeared and the signal-to-noise ratio falls away, creating the characteristic island-shaped feature.

\begin{figure}
    \centering
    \includegraphics[width = 0.9\textwidth]{chapter3/figures/pulsar-cand.jpg}
    \caption[Example of a \texttt{prepfold} plot for the Crab using I-LOFAR.]{Example of a \texttt{prepfold} plot for the Crab using I-LOFAR. The top left panel shows the time averaged pulse profile. The bottom panel is a waterfall phase plot. The middle top panel shows the waterfall frequency plot. The middle bottom plot shows DM trial as a function of $\chi^2$. The top right panel shows $\dot P$  trial values as a function of $\chi^2$. Middle right panel shows $P$ trial values of a function. Bottom right panel shows an example of the `DM island'. }
    \label{fig: prepfold}
\end{figure}

The alternative to period searching is searching for single pulses. This is often used to find non-periodic transients such as FRBs or RRATs as their emission might be missed by conventional period searches. Rather than searching for a repeating signal in the Fourier domain, the dedispersed time series is examined directly for statistically significant peaks above the background noise. A genuine astrophysical pulse will be maximally `uncompressed' at its true DM, producing a significant peak in S/N ratio, the further away from the true DM the more the pulse is broadened by dispersion and the signal is smeared out. These signal pulses are found by simply fitting a box-car function to an RFI clean de-dispersed time series. But the same parameter are noted similar to periodocity searches, no-zero DM and suffcient definition in DM as function of S/N or $\chi^2$ being key signs of a genuine pulse. For more detailed introduction on the methods of finding pulsars please see the following texts \cite{folding_picture, pulsar_handbook, ransom_new_2001}
...
\begin{table}
	\centering
	\caption[Ephemeris values of note for J0523]{Ephemeris values of note for J0523$-$2529. All values are taken from \cite{strader_1fgl_2014} and \cite{halpern_luminous_2022}.}
...
\begin{figure}
    ...
    \caption[Orbital phase and observation coverage of J0523]{\textit{Top:} Phase convention used during this search, with superior conjunction at $\phi = 0.5$. Pulsar shown in blue, companion shown in orange (top down view). \textit{Bottom:}  Bar chart of epochs and corresponding phase coverage of the observation campaign from Parkes (Blue), MeerKAT (Pink) and GBT (Green). }
...
\begin{table}
    ...
    \begin{tabular}{lccccc}
        ...
        \bottomrule
    \end{tabular}
    \caption[Details of J0523 observations]{Details of the search mode observations taken with the Parkes UWL (Project Code: PX094), GBT (Project Codes: AGBT17A-102, AGBT20A-538 and AGBT20A-202) and  TRAPUM \citep{thongmeearkom_targeted_2024}.}
...
\begin{figure}[h]
    ...
    \begin{minipage}[t]{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{chapter3/figures/z_plot.pdf}
        \caption*{(a) }
        \label{fig:z_plot}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{chapter3/figures/radial-velocity-inset.pdf}
        \caption*{(b) }
        \label{fig:radial_velocity}
    \end{minipage}

    \caption[Acceleration search optimisation]{(a) Z-parameter values for various J0523 trial spin periods. The maximum $Z$-value corresponds to the value at the time of ascending node. (b) Radial velocity measurements from \cite{strader_1fgl_2014} as a function of orbital phase. This illustrates the need to separate observations into chunks of $\lesssim P_b/10$ in duration to maintain an approximately linear radial acceleration for periodicity searches.}
    \label{fig:z_and_radial_velocity}
\end{figure}
...
No significant pulsations were detected from the observation campaign. The minimum detectable flux density per channel, $( S_{\text{min,chan}}, \text{in Jy})$ was calculated using Eq. \ref{eq:radiometer_equation_redback} which is an adapted form of the radiometer equation~\citep{pulsar_handbook}: 
...
S_{\min,\rm{chan}}(f, l, b) &= \frac{\left[ T_{\text{sky}}(l,b) + T_{\text{int}}(f) \right] \cdot 2 k_B}{A_{\text{eff}}(f)} \cdot \beta \\ & \times \frac{\text{S/N}_{\min}}{\sqrt{n_p\, t_{\text{obs}}\, B_{\text{channel}}}} \cdot \sqrt{\frac{W}{P - W}}
 \label{eq:radiometer_equation_redback}
\end{split}
\end{equation}
